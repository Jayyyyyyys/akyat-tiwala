...
export default function Home() {
  ...
  // Defensive Check for currentMount
  ...
  async function fetchKuyas() {
    // FIX 1: Early return if currentMount is somehow null when button is clicked
    ...
    <div className="min-h-screen font-sans bg-slate-50">
      <nav className="sticky top-0 bg-emerald-900 text-white p-4 flex justify-between items-center z-50 shadow-md">
        ...
        <div className="space-x-4 text-xs font-bold uppercase">
          ...
          <a href="/portal" className="bg-orange-500 px-3 py-2 rounded">Portal</a>
        </div>
      </nav>
      ...
      <section id="plan" className="py-12 px-4 max-w-3xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {Object.keys(mountains).map((key) => (
                <button 
                  ...
                  // Fixed classname rounded4xl to rounded-3xl for Tailwind compatibility
                  className={`p-5 rounded-3xl font-black uppercase text-sm border-4 transition-all active:scale-95 ${selectedMountain === key ? 'bg-emerald-900 text-white border-emerald-900 shadow-xl' : 'bg-white border-slate-100 text-slate-400 hover:border-emerald-200'}`}
                >
                  {mountains[key]?.name}
                </button>
            ))}
        ...
        {/* Using Optional Chaining ( ?. ) for everything inside currentMount */}
        {currentMount && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
              ...
              <div className="border-t border-slate-50 pt-10">
                ...
                <div className="grid grid-cols-2 bg-emerald-950 p-8 rounded-[2.5rem] shadow-2xl mb-8 border-r-8 border-orange-500">
                  <div className="border-r border-emerald-800 pr-6">
                    <span className="text-[10px] uppercase font-black text-emerald-500 opacity-60 italic leading-none block mb-1 font-sans">Total Budget</span>
                    <p className="text-3xl font-black text-white">₱{totalCost}</p>
                  </div>
                  <div className="pl-6">
                    <span className="text-[10px] uppercase font-black text-orange-400 opacity-80 italic leading-none block mb-1 underline font-sans">Individual Share</span>
            ...
            {guides.length > 0 && (
                ...
                <h3 className="text-center font-black uppercase text-emerald-900">Mga available na Kuyas:</h3>
                {guides.map((kuya) => (
                    ...
                    <div className="w-24 h-24 rounded-full overflow-hidden border-4 border-orange-200 mb-4 bg-slate-100 shadow-lg">
                        <img 
                    ...
                    <div className="text-center mb-6 px-4">
                        ...
                        <p className="text-[10px] font-black bg-emerald-50 text-emerald-600 py-1 px-4 rounded-full uppercase inline-block mb-3 tracking-widest border border-emerald-100">
                        Expert: {kuya.mountains_covered?.join(" • ")}
                        </p>
                    ...
                    <div className="grid grid-cols-2 gap-2 w-full max-w-sm">
                        <a href={`https://m.me/${kuya.messenger_url}`} target="_blank" className="bg-blue-600 text-white text-center py-4 rounded-3xl font-black text-[10px] uppercase italic tracking-widest shadow-lg active:scale-95 transition">Messenger</a>
                        <a href={`tel:${kuya.phone}`} className="bg-slate-900 text-white text-center py-4 rounded-3xl font-black text-[10px] uppercase italic tracking-widest shadow-lg active:scale-95 transition leading-tight">
